...
namespace Omemo
{
    public static class CryptoUtils
    {
        ...
        {
            byte[] buffer = new byte[4]; // 4 bytes = 32 bits = uint
        ...
        public static byte[] SharedSecret(ECPrivKeyModel a, ECPubKeyModel b)
        {
            ...
            agreement.Init(new X25519PrivateKeyParameters(a.key, 0));
    
            byte[] sharedSecret = new byte[agreement.AgreementSize];
            agreement.CalculateAgreement(new X25519PublicKeyParameters(b.key, 0), sharedSecret, 0);
        ...
        public static uint GenerateUniqueDeviceId(List<uint> usedDeviceIds)
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                **/
                id = CryptoUtils.GenerateSecureRandomUInt() % 0x7FFFFFFF; // 2^31-1 
