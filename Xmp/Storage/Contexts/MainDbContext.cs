    ...
    public class MainDbContext: AbstractDbContext, IDisposable
    {
            ...
            if (curDepth < maxDepth)
            {
                foreach (INavigation navigation in entityType.GetNavigations())
                {
                    if (navHirarProps.Add(navigation))
                    {
                        ...
                        GetIncludePathsRec(navHirar, navHirarProps, navigation.TargetEntityType, paths, curDepth + 1, maxDepth);
                        navHirar.Pop();
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<ServerModel>()
                .Property(p => p.ignoredCertificateErrors)
                .HasConversion(
                    v => string.Join(',', v.Select(i => ((int)i.Status).ToString())),
                    v => v.Split(',', StringSplitOptions.RemoveEmptyEntries)
                        .Select(i => new X509ChainStatus { Status = (X509ChainStatusFlags)int.Parse(i) })
                        .ToList());
