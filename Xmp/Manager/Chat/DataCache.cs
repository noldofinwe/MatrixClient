...
namespace Manager.Chat
{
    public class DataCache: AbstractDataTemplate
    {
        ...
        public List<MucInfoModel> GetMucs(string accountBareJid)
        {
            if (initialized)
            {
                ...
                if (countNull > 0)
                {
                    ...
                    // Todo: AppCenterCrashHelper.INSTANCE.TrackError(new ArgumentNullException(), "DataCache.INSTANCE.GetMucs() has MUCs which are null!\nIn theory the DB should prevent something like this...", payload);
        ...
        private static async Task PrepareImageModelPathAndNameAsync(ChatMessageImageReceivedModel img)
        {
            ...
            //Todo: StorageFolder folder = await ConnectionHandler.INSTANCE.IMAGE_DOWNLOAD_HANDLER.GetCacheFolderAsync();
            //Todo: img.targetFolderPath = folder.Path;
        }
