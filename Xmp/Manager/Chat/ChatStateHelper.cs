...
namespace Manager.Chat
{
    ...
    public class ChatStateHelper: IDisposable
    {
        ...
        private void CancelTimer(ref Timer timer)
        {
            if (!(timer is null))
            {
                timer.Dispose();
        ...
        private void StartOwnTimer(TimeSpan timeout)
        {
            ...
            ownTimer = new Timer(_ => onOwnTimeout(), null, timeout, timeout);
        }
        ...
        private void StartOtherTimer(TimeSpan timeout)
        {
            ...
            otherTimer = new Timer(_ => onOtherTimeout(), null, timeout, timeout);
        }
        ...
        private void onOwnTimeout()
        {
            if (ownTimer is null || disposed)
            {
                return;
            }
        ...
        private void onOtherTimeout()
        {
            if (otherTimer is null || disposed)
            {
                return;
            }
