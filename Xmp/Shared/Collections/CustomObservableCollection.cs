    ...
    public class CustomObservableCollection<T>: AbstractDataTemplate, IObservableList<T>
    {
        ...
        protected async void OnCollectionChanged(NotifyCollectionChangedEventArgs e)
        {
            ...
            try
            {
                if (invokeInUiThread)
                {
                   //Todo: await SharedUtils.CallDispatcherAsync(() => CollectionChanged?.Invoke(this, e));
                }
                else
                {
            ...
            catch (Exception ex)
            {
               ...
               //Todo: AppCenterCrashHelper.INSTANCE.TrackError(ex, "Failed to trigger CustomObservableCollection.OnCollectionChanged()");
            }
        }
