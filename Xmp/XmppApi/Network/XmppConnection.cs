...
using XmppApi.Network.XML.Messages.Helper;
using XmppApi.Network.XML.Messages.Processor;
using XmppApi.Network.XML.Messages.XEP_0048;
...
namespace XmppApi.Network
{
    public class XmppConnection: AbstractConnection, IDisposable, IMessageSender
    {
        ...
        public XmppConnection(XMPPAccount account) : base(account)
        {
            ...
            //Todo: NetworkHelper.Instance.NetworkChanged += Instance_NetworkChanged;
        }
        ...
        private void ResetConnectionTimer()
        {
            ...
            connectionTimer = new Timer(async _ => await OnConnectionTimerTimeoutAsync(), null, timeout, Timeout.InfiniteTimeSpan);        }

        /// <summary>
        /// Called once the connection timer got triggered.
        /// </summary>
        private Task OnConnectionTimerTimeoutAsync()
        {
        ...
        {
            if (TcpConnection.IsInternetAvailable())
            {
                if (state == ConnectionState.DISCONNECTED || state == ConnectionState.ERROR)
                {
