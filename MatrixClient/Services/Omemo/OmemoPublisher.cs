...
public class OmemoPublisher
{
  ...
  public async Task PublishBundle(Jid to)
  {
    ...
    var result = await client.PublishItemAsync(to.Bare, node, item);

    var pubsub = new PubSubManager(client);
    await pubsub.EnsureDeviceIdPublishedAsync(to, deviceId);

    var test = await pubsub.GetDeviceListAsync(to.Bare);
  }
